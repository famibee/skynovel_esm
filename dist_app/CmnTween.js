import { k as _, e as $, d as x } from "./app2.js";
import { u as P, E as i, b as L, T as S, a as O } from "./Reading.js";
const h = `trans
`, w = "tsy nm:";
class F {
  static #t = {};
  ...
  static cnvTweenArg(t, n) {
    ...
    for (const e of Object.keys(this.hMemberCnt)) {
      ...
      if (!f) continue;
      const [d, a] = f.split(","), u = s[e] = parseFloat(d);
      a && (s[e] += Math.round(
  ...
  static tween(t, n, s, e, c, r, p, f = !0) {
    const d = this.#s.isSkipping ? 0 : _(n, "time", NaN), a = new S(s).to(e, d).onUpdate(c);
    ...
    if (u) {
      ...
      for (const { groups: o } of u.matchAll(this.#c)) {
        const { x: T, x2: b, y: g, y2: Q, o: E, o2: k, json: I } = o;
        let l = {};
        if (I) try {
          l = JSON.parse(I);
        } catch (B) {
          console.error(`🍝 json=${I} ` + String(B));
          continue;
        }
        else
        ...
        I ?? `{x:${T} y:${g} o:${E}}`} => hTo:${JSON.stringify(v)}`);
        const C = new S(s).to(v, d);
    ...
    y.onComplete(() => {
      const o = this.#t[t];
      o?.tw && (delete this.#t[t], o.tw = void 0, a.stop(), o.onEnd?.(), r(), O.notifyEndProc(w + t));
    });
    const { chain: m } = n;
    if (m) {
      const o = this.#t[m];
  ...
  */
  ...
  static wt(t) {
    ...
    const s = () => this.#a();
    return O.beginProc(w + h, s, !0, s), !0;
  }
  // レイヤのトランジションの停止
  static #a() {
  ...
  static async closeTrans() {
    ...
    O.beginProc(w + h, s, !1, s), this.#a(), await n;
  }
  // トゥイーン終了待ち
  static wait_tsy(t) {
    ...
    if (!s?.tw) return !1;
    const e = () => s.tw?.end();
    return O.beginProc(w + n, e, !0, e), !0;
  }
  static #n(t) {
